...
ELEMENT_POLL_SPEED = 1500;
POSITIVE_COLOR = 'lightgreen';
NEGATIVE_COLOR = 'lightpink';
...
function main() {
	...
	j('#mw-content-text p').each(function(index) {
	
		// Get HTML content and split on whitespaces
		var html         = j(this).html();
		var tokens       = html.split(/\s*[\s,]\s*/);
		var replace_html = '';
		var replacements = 0;
		var pos_score    = 0;
		var neg_score    = 0;
		...
		for (var token_i = 0; token_i < tokens.length; token_i++) {
			...
			// If a token is in the list of tokens to be highlighted,
			// Set a background color
			var color = '';
			if (j.inArray(token, POSITIVE_WORDS) >= 0) {
				color = POSITIVE_COLOR;
				pos_score++;
			} else if (j.inArray(token, NEGATIVE_WORDS) >= 0) {
			...
			if (color != '') {
				replace_html += '<span style="background-color:' + color
		...
		// Based on paragraph score, highlight the whole paragraph
		if (pos_score > neg_score) {
			j(this).css('background-color', '#f0fff0');
		} else if (neg_score > pos_score) {
			j(this).css('background-color', '#fff0f0');
		}
		
		// Insert the highlighted content into html
		if (replacements > 0) {
			j(this).html(replace_html);
		}
...
Hu, M. & Liu, B. (2004a). Mining and Summarizing Customer Reviews. Proceedings of KDD'04, 168-177.
*/
NEGATIVE_WORDS = [':(', ';_;', 'T_T', '=(', ':*(', ')-:', ':-(', ':<', ')*:',
